...
check(
  "auth via proxy",
  auth.err === 0,
  auth.err === 0 ? `token ok, expire=${auth.dat.expire}s` : JSON.stringify(auth)
);
if (auth.err !== 0) {
...
check(
  "plant info",
  info.err === 0,
  `${info.dat?.name} · ${info.dat?.nominalPower} kW · ${info.dat?.address?.city} · status=${info.dat?.status} · tariff=₹${info.dat?.profit?.unitProfit}`
);

const sum = await call(
  `&action=queryPlantCurrentData&plantid=${PID}&par=ENERGY_TODAY,ENERGY_MONTH,ENERGY_YEAR,ENERGY_TOTAL`,
  secret,
  token
);
...
check(
  "energy summary",
  sum.err === 0,
  `today=${g("ENERGY_TODAY")} month=${g("ENERGY_MONTH")} year=${g("ENERGY_YEAR")} total=${g("ENERGY_TOTAL")} kWh`
);

const today = new Date().toISOString().slice(0, 10);
const curve = await call(
  `&action=queryPlantActiveOuputPowerOneDay&plantid=${PID}&date=${today}`,
  secret,
  token
);
...
check(
  "day curve",
  curve.err === 0 && pts.length === 288,
  `${pts.length} pts, peak ${Math.max(...pts.map((p) => parseFloat(p.val)))} kW`
);

const mp = await call(
  `&action=queryPlantEnergyMonthPerDay&plantid=${PID}&date=${today.slice(0, 7)}`,
  secret,
  token
);
...
const ym = await call(
  `&action=queryPlantEnergyYearPerMonth&plantid=${PID}&date=${today.slice(0, 4)}`,
  secret,
  token
);
...
const warn = await call(
  `&action=queryPlantWarning&plantid=${PID}&i18n=en_US&page=0&pagesize=100`,
  secret,
  token
);
...
check(
  "warnings",
  warn.err === 0,
  `${warn.dat?.warning?.length}/100 fetched (total ${warn.dat?.total}), ${outages.length} grid-outage events on page 0`
);
