...
import {
  getDayData, getMonthData, getYearData, SYSTEM_KWP, TARIFF_RS_PER_KWH,
  LIFETIME, MONTH_SHORT, MONTH_LONG, fmtRs, fmtDate, INSTALL_DATE,
  getDayOutages, getMonthOutages, getYearOutages, SYSTEM_COST_RS,
  getTomorrowForecast, getCleaningNudge,
  prefetchFor, getCity, getStatusInfo, getUpdatedLabel, getCurrentKw,
} from "@/lib/data";
import type { OutageEvent } from "@/lib/data";
import { downloadReport, type PeriodReport } from "@/lib/report";
import { downloadShareCard } from "@/lib/shareCard";
import { AlertTriangle, Cloud, CloudSun, Droplets, Leaf, Share2, Sun, TreePine } from "lucide-react";
...
function buildCurvePath(
  ...
  const inOutage = (h: number) =>
    outages.some((o) => h >= o.startH && h < (o.endH ?? nowH ?? 24));
  ...
  const pts = [...hs].sort((a, b) => a - b).map((h) => ({
    x: (h / 23) * width,
    ...
    <>
      {events.map((o, i) => (
        <div key={i} className="flex items-baseline justify-between gap-3">
          <span className="text-[13px] font-medium [font-variant-numeric:tabular-nums]" style={{ color: INK }}>
            {fmtTimeRange(o.startH, o.endH ?? o.startH)} · {fmtDuration(o.startH, o.endH ?? o.startH)}
          </span>
          <span className="text-[12px] whitespace-nowrap [font-variant-numeric:tabular-nums]" style={{ color: DEEP_GOLD }}>
            {o.lostKwh > 0 ? `≈${o.lostKwh} kWh lost (≈${fmtRs(o.lostRs)})` : "No production impact"}
          </span>
        </div>
      ))}
    </>
...
function Card({ children, className = "", testId }: { children: React.ReactNode; className?: string; testId?: string }) {
  return (
    <div data-testid={testId} className={`bg-white rounded-[18px] shadow-[0_2px_14px_rgba(13,32,29,0.05)] border p-5 ${className}`} style={{ borderColor: BORDER }}>
      {children}
    </div>
  );
}
...
function Label({ children }: { children: React.ReactNode }) {
  return <p className="text-[12px] font-medium tracking-wide" style={{ color: MUTED }}>{children}</p>;
}

function StatCard({ label, value, unit, note, valueColor, testId, icon }: {
  label: string; value: string; unit?: string; note: string; valueColor?: string; testId?: string; icon?: React.ReactNode;
}) {
  return (
    <Card testId={testId}>
      ...
      <p className="mt-1 text-[28px] font-medium tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY, color: valueColor ?? INK }}>
        {value}
        {unit && <span className="text-[14px]" style={{ color: MUTED }}> {unit}</span>}
      </p>
      <p className="text-[12px]" style={{ color: FAINT }}>{note}</p>
    </Card>
  );
}

function Bars({ values, highlight, labels, marks }: {
  values: number[]; highlight: number; labels?: string[]; marks?: number[];
}) {
      ...
      {values.map((v, i) => (
        <div key={i} className="flex flex-1 flex-col items-center gap-1.5 min-w-0">
          <div
            ...
            style={{ height: `${Math.max((v / max) * 84, 3)}px`, background: i === highlight ? GOLD : PALE_TEAL }}
          />
          {marks && (
          ...
          {labels && (
            <span className="text-[10px] truncate" style={{ color: i === highlight ? INK : FAINT, fontWeight: i === highlight ? 600 : 400 }}>
              {labels[i]}
            </span>
          )}
        </div>
...
export default function Home() {
  ...
  const month = useMemo(() => getMonthData(anchor.getFullYear(), anchor.getMonth(), tariff), [anchor, tariff]);
  ...
  const periodLabel =
    mode === "day" ? (isToday ? "Today" : fmtDate(anchor))
    : mode === "month" ? `${MONTH_LONG[anchor.getMonth()]} ${anchor.getFullYear()}${isCurrentMonth ? " · so far" : ""}`
    : `${anchor.getFullYear()}${isCurrentYear ? " · so far" : ""}`;

  const percentile = mode === "day" ? day.percentile : mode === "month" ? month.percentile : year.percentile;
  ...
  async function handleDownload() {
    const report: PeriodReport =
      mode === "day"
        ...
        : mode === "month"
          ? { kind: "month", label: `${MONTH_LONG[anchor.getMonth()]} ${anchor.getFullYear()}`, year: anchor.getFullYear(), month: anchor.getMonth(), data: month }
          : { kind: "year", label: `Year ${anchor.getFullYear()}`, year: anchor.getFullYear(), data: year };
  ...
  return (
    <div className="min-h-screen antialiased" style={{ background: "#f3f1ea", color: INK, fontFamily: BODY }}>
      {navBusy && (
        <div data-testid="nav-loading" className="fixed top-0 left-0 right-0 z-50 h-[3px]" style={{ background: `linear-gradient(90deg, ${GOLD}, ${DEEP_GOLD})` }} />
      )}
      <div className="mx-auto max-w-md px-5 pb-28 pt-8 space-y-4">
        ...
        <header className="flex items-start justify-between px-1">
          <div>
            <p className="text-[11px] font-semibold tracking-[0.18em] uppercase" style={{ color: DEEP_GOLD }}>
              Greentek Alliance
            </p>
            <h1 className="text-[30px] font-semibold tracking-tight leading-tight mt-0.5" style={{ fontFamily: DISPLAY }}>
              My Solar
            </h1>
            <p className="text-[13px]" style={{ color: MUTED }}>{fmtDate(today)} · {getCity()}</p>
          </div>
          <div className="flex items-center gap-1.5 rounded-full px-3 py-1.5 mt-2" style={{ background: statusInfo.ok ? PALE_TEAL : PALE_GOLD }}>
            <span className="h-2 w-2 rounded-full" style={{ background: statusInfo.ok ? BRIGHT_TEAL : GOLD }} />
            <span className="text-[12px] font-semibold" style={{ color: statusInfo.ok ? TEAL : DEEP_GOLD }}>{statusInfo.label}</span>
          </div>
        </header>
        ...
        <div className="flex items-center gap-1 rounded-full bg-white border p-1" style={{ borderColor: BORDER }}>
        ...
        <div className="flex items-center justify-between px-1">
          <button onClick={() => shift(-1)} className="flex h-9 w-9 items-center justify-center rounded-full bg-white border text-[16px] active:scale-[0.98] transition-transform duration-150" style={{ borderColor: BORDER, color: INK }} aria-label="Previous">
            ‹
          </button>
          <button
          ...
          >
            <span className="text-[15px] font-semibold" style={{ fontFamily: DISPLAY }}>{periodLabel}</span>
            <svg viewBox="0 0 24 24" fill="none" stroke={FAINT} strokeWidth="2.4" strokeLinecap="round" className="h-3 w-3">
              <path d="M6 9l6 6 6-6" />
            </svg>
          </button>
            ...
            {!canGoNext ? null : (
              <button
                ...
                onClick={() => {
                  ...
                  else if (mode === "month") void goTo(mode, new Date(today.getFullYear(), today.getMonth(), 1));
          ...
          <>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <p className="flex items-center gap-1.5 text-[12px] font-medium" style={{ color: MUTED }}>
                      <span className="h-2 w-2 rounded-full" style={{ background: BRIGHT_TEAL }} />
                      Producing now
                    </p>
                    <p className="mt-1 text-[30px] font-medium tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY }}>
                      {heroKw}<span className="text-[14px]" style={{ color: MUTED }}> kW</span>
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-[12px] font-medium" style={{ color: MUTED }}>Today</p>
                    <p className="mt-1 text-[30px] font-medium tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY }}>
                      {day.kwh}<span className="text-[14px]" style={{ color: MUTED }}> kWh</span>
                    </p>
                  </div>
                </div>
                <div className="mt-3.5 h-1.5 overflow-hidden rounded-full" style={{ background: PALE_TEAL }}>
                  <div className="h-full rounded-full" style={{ width: `${heroPct}%`, background: `linear-gradient(90deg, ${GOLD}, ${DEEP_GOLD})` }} />
                </div>
                <div className="mt-2 flex items-baseline justify-between">
                  <p className="text-[11px]" style={{ color: FAINT }}>{heroPct}% of your {SYSTEM_KWP} kW system</p>
                  <p className="text-[13px] font-semibold [font-variant-numeric:tabular-nums]" style={{ color: TEAL }}>{fmtRs(day.savedRs)} saved</p>
                </div>
              </Card>
            )}
            ...
            {isToday && cleaningNudge && (
              ...
              >
                ...
                <div>
                  <p className="text-[14px] font-semibold leading-snug [font-variant-numeric:tabular-nums]" style={{ color: INK }}>
                    Generation is {cleaningNudge.belowPct}% below normal this week — a panel cleaning may help.
                  </p>
            ...
            <Card>
              <div className="flex items-baseline justify-between">
                ...
                <p className="text-[12px]" style={{ color: MUTED }}>Peak {day.peakKw} kW · {day.peakTime}</p>
              </div>
              <svg viewBox={`0 0 ${W} ${H + 6}`} className="mt-3 w-full">
                ...
                {outages.map((o, i) => {
                  ...
                  return (
                    <g key={i}>
                      <rect x={x1} y="0" width={Math.max(x2 - x1, 1)} height={H} fill="rgba(180,35,24,0.10)" />
                      <line x1={x2} y1="0" x2={x2} y2={H} stroke="#b42318" strokeWidth="1" strokeDasharray="3 3" />
                    </g>
                  );
                })}
                ...
                {isToday && (
                  <>
                    <line x1={nowX} y1="4" x2={nowX} y2={H} stroke="#d8d3c4" strokeDasharray="3 3" strokeWidth="1" />
                    <circle cx={nowX} cy={nowY} r="3.5" fill={GOLD} stroke="#fff" strokeWidth="1.5" />
                  </>
                )}
              </svg>
              ...
              >
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {outages.length > 1 && (
                    ...
                    >
                      <span className="text-[13px] font-semibold" style={{ color: INK }}>Total</span>
                      <span className="text-[12px] font-semibold [font-variant-numeric:tabular-nums]" style={{ color: DEEP_GOLD }}>
                        ≈{Math.round(outages.reduce((a, o) => a + o.lostKwh, 0) * 10) / 10} kWh lost (≈
            ...
            {isToday && (
              <Card testId="forecast-card" className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[15px] font-medium [font-variant-numeric:tabular-nums]" style={{ color: INK }}>
                    Tomorrow: {forecast.condition} — expect ≈{forecast.expectedKwh} kWh
                  </p>
                  <p className="text-[12px]" style={{ color: MUTED }}>{forecast.note}</p>
                </div>
              </Card>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
              {!isToday && (
                <>
                  <StatCard label="Energy" value={`${day.kwh}`} unit="kWh" note={`${day.sunHours} peak sun hours`} />
                  <StatCard label="Saved" value={fmtRs(day.savedRs)} note={`at ${fmtRs(tariff)} / kWh`} valueColor={TEAL} />
                </>
              )}
              <StatCard label="CO₂ avoided" value={`${day.co2Kg}`} unit="kg" note="vs grid electricity" icon={<Leaf className="h-4 w-4" style={{ color: TEAL }} />} />
              <StatCard label="Trees equivalent" value={`${(day.co2Kg / 20).toFixed(1)}`} note="planted this day" icon={<TreePine className="h-4 w-4" style={{ color: TEAL }} />} />
            </div>
          </>
        )}
          ...
          <>
            <Card>
              <div className="flex items-baseline justify-between">
                ...
                <p className="text-[13px] font-semibold [font-variant-numeric:tabular-nums]" style={{ color: TEAL }}>{fmtRs(month.savedRs)} saved</p>
              </div>
              <p className="mt-1 text-[34px] font-medium tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY }}>
                {month.kwh.toLocaleString("en-IN")} <span className="text-[16px]" style={{ color: MUTED }}>kWh</span>
              </p>
              <Bars
              ...
              />
              <p className="mt-3 text-[12px] [font-variant-numeric:tabular-nums]" style={{ color: FAINT }}>
                Best day: {month.bestDay.day} {MONTH_SHORT[anchor.getMonth()]} · {month.bestDay.kwh} kWh
              </p>
            </Card>
            ...
            {monthOutages.count > 0 && (
              <div data-testid="month-outage-summary" className="flex items-center gap-2 px-1">
                ...
                <p className="text-[12px] [font-variant-numeric:tabular-nums]" style={{ color: MUTED }}>
                  {monthOutages.count} outage{monthOutages.count === 1 ? "" : "s"} · {monthOutages.totalHours} h total · ≈
                  {monthOutages.lostKwh} kWh lost (≈{fmtRs(monthOutages.lostRs)})
                </p>
              </div>
            )}

            <div className="grid grid-cols-2 gap-4">
              <StatCard label="Daily average" value={`${Math.round((month.kwh / Math.max(month.days.length, 1)) * 10) / 10}`} unit="kWh" note={`across ${month.days.length} days`} />
              <StatCard label="CO₂ avoided" value={`${(month.co2Kg / 1000).toFixed(2)}`} unit="t" note={`≈ ${Math.round(month.co2Kg / 20)} trees`} />
            </div>
            ...
            <button
              ...
              style={{ background: "rgba(201,164,96,0.16)", border: "1px solid rgba(138,104,38,0.25)", color: INK }}
            >
            ...
            {monthOutages.count > 0 && (
              <Card className="p-0 overflow-hidden">
                ...
                >
                  <span className="text-[13px] font-medium" style={{ color: MUTED }}>Outages this month</span>
                  <span className="text-[13px]" style={{ color: FAINT }}>{showOutages ? "Hide" : "Show"}</span>
                </button>
                {showOutages && (
                  <div className="border-t px-5 py-4 space-y-2.5" style={{ borderColor: "#ece8dc" }}>
                    {/* Total at top */}
                    <div className="flex items-baseline justify-between gap-3 pb-2.5 border-b" style={{ borderColor: "#ece8dc" }}>
                      <span className="text-[13px] font-semibold" style={{ color: INK }}>
                        Total · {monthOutages.count} outage{monthOutages.count > 1 ? "s" : ""} · {monthOutages.totalHours} h
                      </span>
                      <span className="text-[13px] font-semibold whitespace-nowrap [font-variant-numeric:tabular-nums]" style={{ color: "#b42318" }}>
                        ≈{monthOutages.lostKwh} kWh (≈{fmtRs(monthOutages.lostRs)})
                      </span>
                    </div>
                    {monthOutages.affectedDays.map((d) => (
                      <div key={d} className="space-y-2">
                        {getDayOutages(new Date(anchor.getFullYear(), anchor.getMonth(), d), tariff).map((o, i) => (
                          <div key={i} className="flex items-baseline justify-between gap-3">
                            <span className="text-[13px] [font-variant-numeric:tabular-nums]" style={{ color: MUTED }}>
                              {d} {MONTH_SHORT[anchor.getMonth()]} · {fmtTimeRange(o.startH, o.endH ?? o.startH)} · {fmtDuration(o.startH, o.endH ?? o.startH)}
                            </span>
                            <span className="text-[13px] font-medium whitespace-nowrap [font-variant-numeric:tabular-nums]" style={{ color: INK }}>
                              {o.lostKwh > 0 ? `≈${o.lostKwh} kWh (≈${fmtRs(o.lostRs)})` : "No impact"}
                            </span>
                          </div>
                        ))}
                      </div>
          ...
          <>
            <Card>
              <div className="flex items-baseline justify-between">
                ...
                <p className="text-[13px] font-semibold [font-variant-numeric:tabular-nums]" style={{ color: TEAL }}>{fmtRs(year.savedRs)} saved</p>
              </div>
              <p className="mt-1 text-[34px] font-medium tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY }}>
                {year.kwh.toLocaleString("en-IN")} <span className="text-[16px]" style={{ color: MUTED }}>kWh</span>
              </p>
              <Bars
              ...
              />
              <p className="mt-3 text-[12px] [font-variant-numeric:tabular-nums]" style={{ color: FAINT }}>
                Best month: {MONTH_LONG[year.bestMonth.month]} · {year.bestMonth.kwh.toLocaleString("en-IN")} kWh
              </p>
            </Card>
            ...
            {yearOutages.count > 0 && (
              <div data-testid="year-outage-summary" className="flex items-center gap-2 px-1">
                <span className="h-2 w-2 shrink-0 rounded-full" style={{ background: GOLD }} />
                <p className="text-[12px] [font-variant-numeric:tabular-nums]" style={{ color: MUTED }}>
                  {yearOutages.count} outage{yearOutages.count === 1 ? "" : "s"} · {yearOutages.totalHours} h · ≈
                  {yearOutages.lostKwh} kWh lost (≈{fmtRs(yearOutages.lostRs)})
                </p>
              </div>
            )}

            <div className="grid grid-cols-2 gap-4">
              <StatCard label="Grid uptime" testId="year-uptime" value={`${yearOutages.uptimePct}`} unit="%" note="this year" valueColor={DEEP_GOLD} />
              <StatCard label="CO₂ avoided" value={`${(year.co2Kg / 1000).toFixed(2)}`} unit="t" note={`≈ ${Math.round(year.co2Kg / 20)} trees`} />
            </div>
          </>
        )}
        ...
        <Card className="relative">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-full px-2.5 py-1 text-[11px] font-semibold" style={{ background: PALE_GOLD, color: DEEP_GOLD }}>
              Top {100 - percentile}%
            </span>
          </div>
          <p className="mt-2 text-[15px] leading-snug">
            Better than <span className="font-semibold" style={{ color: TEAL }}>{percentile}%</span> of similar{" "}
            {SYSTEM_KWP} kW plants in {getCity()} {mode === "day" ? "today" : mode === "month" ? "this month" : "this year"}.
          </p>
          {tipOpen && (
            <div
            ...
            >
              <p className="text-[13px] font-semibold mb-1" style={{ fontFamily: DISPLAY }}>How this is calculated</p>
              <p className="text-[12px] leading-relaxed" style={{ color: MUTED }}>
                We compare your plant's actual generation against its expected output for its size, location and the
                weather — then rank it against all {SYSTEM_KWP} kW residential plants monitored by Greentek in your
                state. A higher percentile means your system is converting sunlight better than most, factoring out
                weather you can't control.
              </p>
              <button onClick={() => setTipOpen(false)} className="mt-2 text-[12px] font-semibold" style={{ color: DEEP_GOLD }}>
                Got it
              </button>
            </div>
          )}
        ...
        <Card className="p-0 overflow-hidden">
          <div className="px-5 py-4 text-center" style={{ background: "#f7f5ee" }}>
            <p className="text-[12px]" style={{ color: MUTED }}>Since installation · {fmtDate(INSTALL_DATE)}</p>
            <p className="text-[19px] font-semibold tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY }}>
              {(LIFETIME.kwh / 1000).toFixed(1)} MWh · <span style={{ color: DEEP_GOLD }}>{fmtRs(Math.round(LIFETIME.kwh * tariff))} saved</span>
            </p>
            <p className="text-[12px] mt-0.5 [font-variant-numeric:tabular-nums]" style={{ color: FAINT }}>
              {(LIFETIME.co2Kg / 1000).toFixed(1)} t CO₂ · ≈ {Math.round(LIFETIME.co2Kg / 20)} trees
            </p>
          </div>
        </Card>
        ...
        <Card testId="payback-card">
          ...
          <p className="mt-1 text-[26px] font-medium tracking-tight [font-variant-numeric:tabular-nums]" style={{ fontFamily: DISPLAY, color: INK }}>
            {fmtRs(payback.recovered)} recovered
          </p>
          <p className="text-[12px] [font-variant-numeric:tabular-nums]" style={{ color: FAINT }}>
            {payback.pct}% of {fmtRs(SYSTEM_COST_RS)} · break-even ≈ {MONTH_LONG[payback.breakeven.getMonth()]} {payback.breakeven.getFullYear()}
          </p>
          <div className="mt-3 h-2 overflow-hidden rounded-full" style={{ background: PALE_TEAL }}>
            <div
              className="h-full rounded-full"
              style={{ width: `${payback.pct}%`, background: `linear-gradient(90deg, ${GOLD}, ${DEEP_GOLD})` }}
            />
          </div>
        </Card>
        ...
        <Card className="p-0 overflow-hidden">
          <button data-testid="technical-details-toggle" onClick={() => setShowAdvanced(!showAdvanced)} className="flex w-full items-center justify-between px-5 py-4">
            <span className="text-[13px] font-medium" style={{ color: MUTED }}>Technical details</span>
            <span className="text-[13px]" style={{ color: FAINT }}>{showAdvanced ? "Hide" : "Show"}</span>
          </button>
          {showAdvanced && (
            <div className="border-t px-5 py-4 space-y-2.5" style={{ borderColor: "#ece8dc" }}>
              {[
                ...
                <div key={k} className="flex items-center justify-between">
                  <span className="text-[13px]" style={{ color: MUTED }}>{k}</span>
                  <span className="text-[13px] font-medium" style={{ color: INK }}>{v}</span>
                </div>
              ))}
            </div>
          )}
        ...
        >
          <p className="text-[15px] font-semibold" style={{ fontFamily: DISPLAY, color: "#3d2f10" }}>
            You've saved {fmtRs(Math.round(LIFETIME.kwh * tariff))} and planted {Math.round(LIFETIME.co2Kg / 20)} trees
          </p>
          <p className="mt-1 text-[13px]" style={{ color: "#5c4a1e" }}>
            Know a neighbour who'd love numbers like these?
          </p>
          <button className="mt-3 rounded-full px-6 py-2.5 text-[14px] font-semibold text-white" style={{ background: INK }}>
            Refer & Earn
          </button>
        </div>
          ...
          {(
            [
              { id: "solar", label: "Solar", icon: <SunIcon className="h-6 w-6" /> },
              {
                ...
                label: "Settings",
                icon: (
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" className="h-6 w-6">
      ...
      {pickerOpen && (
        <PeriodPicker mode={mode} anchor={anchor} onSelect={(d) => void goTo(mode, d)} onClose={() => setPickerOpen(false)} />
      )}
    </div>
  );
}
